...
function markOcrFailed(filepath) {
  const db = getDb()
  db.prepare(`UPDATE screenshots SET ocr_status = 'failed' WHERE filepath = ?`).run(filepath)
}
...
function markAiFailed(filepath) {
  const db = getDb()
  db.prepare(`UPDATE screenshots SET ai_status = 'failed' WHERE filepath = ?`).run(filepath)
}
