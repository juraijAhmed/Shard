...
function resolveNaturalDateRange(query) {
  ...
  if (q === "today") return { from: startOf(now), to: endOf(now) };
...
function searchByDate(query, allScreenshots) {
  ...
  if (/^20\d{2}$/.test(q)) {
    ...
    return allScreenshots.filter(
      (s) => new Date(s.timestamp).getFullYear() === year,
    );
  }
...
ipcMain.handle("screenshots:semanticSearch", async (_, query) => {
  ...
  if (range) {
    return getAllScreenshots().filter(
      (s) => s.timestamp >= range.from && s.timestamp <= range.to,
    );
  }
  ...
  const confidentVectorResults = vectorResults.filter(
